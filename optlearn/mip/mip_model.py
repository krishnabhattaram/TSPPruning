...
class tspProblem():
    ...
    def optimise_scip(self, max_nodes=1e10, max_rounds=1e10):
        ...
        if self.times_optimised < 1:
            contraint_handler = constraints.tsp_constraint_handler(self,
...
class vrpProblem():
    """ This is designed to find solutions of IP VRPs """
    ...
    def __init__(self, graph,
        ...
        self.fetch_demands(graph)
        self.fetch_depots(graph)
        self.fetch_vehicle_number(graph)
        self.fetch_fleet_info(graph)
        self.zero_depot_demands()
    ...
    def fetch_demands(self, graph):
        """ Grab the demands from the graph """

        self.demands = nx.get_node_attributes(graph, "demand")

    def fetch_depots(self, graph):
        """ Fetch a list of the depot nodes (only works for one currently) """

        self.depots = [key for key in graph.nodes if key not in self.demands.keys()]

    def fetch_vehicle_number(self, graph):
        """ Fetch the number of vehciles from the graph """

        self.vehicle_num = graph.graph["vehicle_num"]

    def fetch_fleet_info(self, graph):
        """ Fetch information about the vehicles """

        self.fleet_info = graph.graph["fleet"]

    def zero_depot_demands(self):
        """ Set the depot demands to zero """

        for depot in self.depots:
            self.demands[depot] = 0

    def get_capacity(self):
        """ Get the capacity of the first vehicle """

        key = sorted(list(self.fleet_info.keys()))[0]
        return self.fleet_info[key]["capacity"]
    ...
    def set_edge_variables(self, graph):
        """ Create and set all VRP edge variables """
    ...
    def set_degree_constraint(self, vertex):
        ...
        if self._is_symmetric:
            lhs, rhs = self.get_variables_sum(vertex), 2
            if self.verbose:
                print("Degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==")
        if self._is_asymmetric:
            lhs, rhs = self.get_inward_variables_sum(vertex), 1
            if self.verbose:
                print("Inward degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==")
            lhs, rhs = self.get_outward_variables_sum(vertex), 1
            if self.verbose:
                print("Outward degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==")

    def set_depot_degree_constraint(self, vertex):
        """ Set the degree constraint for the given depot """

        if self._is_symmetric:
            lhs =  self.get_variables_sum(vertex)
            rhs = mip_utils.get_depot_uses(vertex, self.fleet_info)
            if self.verbose:
                print("Depot degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==") 
        if self._is_asymmetric:
            lhs =  self.get_inward_variables_sum(vertex)
            rhs = mip_utils.get_depot_entries(vertex, self.fleet_info)
            if self.verbose:
                print("Inward depot degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==") 
            lhs =  self.get_outward_variables_sum(vertex)
            rhs = mip_utils.get_depot_exits(vertex, self.fleet_info)
            if self.verbose:
                print("Outward depot degree constraint:\n {} {} {}\n".format(lhs, "==", rhs))
            self.set_constraint(lhs, rhs, "==") 

    def set_degree_constraints(self):
        """ Define the degree constraints for all non-depot vertices """

        for vertex in self.vertices:
            if vertex not in self.depots:
                self.set_degree_constraint(vertex)

    def set_depot_degree_constraints(self):
        """ Define the degree constraints for all the depot vertex """

        for vertex in self.depots:
            self.set_depot_degree_constraint(vertex)
    ...
    def set_dantzig_constraints(self):
        ...
        if self.verbose:
            print("Setting degree constraints... \n")
        self.set_degree_constraints()
        self.set_depot_degree_constraints()
        if self.verbose:
            print("Degree constraints set... \n")
    ...
    def optimise_scip(self, max_nodes=1e10, max_rounds=1e10):
        ...
        if self.times_optimised < 1:
            contraint_handler = constraints.vrp_constraint_handler(self,
                                                                    max_rounds=max_rounds)
            self.problem.includeConshdlr(contraint_handler,
                                         "VRP", "Capacity Cut",
                                         sepapriority = -1, enfopriority = -1,
    ...
    def optimise(self, max_nodes=1e10, max_rounds=1e10):
        ...
        if self._solver == "xpress":
            raise NotImplementedError("Not implemented for Xpress yet!")
        if self._solver == "scip":
            self.optimise_scip(max_nodes=max_nodes, max_rounds=max_rounds)
        if self._solver == "coin":
            raise NotImplementedError("Not implemented for COIN yet!")
